          ...
          <div class="roster-card">
            ...
            <div class="rc-info">
              ...
              <div class="rc-tabs">
                ...
                ${p.live ? `<button class="rc-tab" onclick="showPanel(event, ${i}, 'voice')">
                  <span class="icon circle"></span><span class="lbl">CHAT VOICE</span>
                  <b id="rc-voice-${i}">—</b>
                </button>` : ''}
              </div>
            </div>
...
function showPanel(event, i, view) {
  ...
  panel.querySelector('.rp-body').innerHTML =
    ...
    : view === 'voice' ? voicePanel(player)
    : livePanel(player);
  panel.classList.add('on');
  markTabs(i, view);
  // Every other panel is a readout. This one has controls, so it needs wiring
  // up after its markup lands.
  if (view === 'voice') initVoicePanel(panel, player);
}

const PANEL_TITLES = { bag: 'INVENTORY', attr: 'ATTRIBUTES', live: 'SERVER RECORD', voice: 'CHAT VOICE' };
...
// -- Chat voices -------------------------------------------------------
//
// What the server says out loud when this player types. The pick has to reach
// the game server, which this site cannot talk to: it sits behind a managed
// panel with no inbound access. So the TTS service holds the shared player ->
// voice list and the mod polls it, and everything below talks to that service
// directly using a short lived token this site signs. See minecraft/voices.py
// for why the browser rather than the server makes those calls.
//
// Anything coming back from the service was typed by a player, so it goes in
// with textContent and never through innerHTML.

const VOICE_CACHE = { meta: null, remote: null, at: 0 };

function voicePanel(p) {
  // Static shell only. Everything with a name in it is built in JS below.
  return `
    <div class="voice-panel" data-uuid="${p.uuid}">
      <div class="vp-body"><p class="rp-none">loading&hellip;</p></div>
      <div class="vp-status"></div>
    </div>`;
}

function vpCookie(name) {
  const hit = document.cookie.split('; ').find(c => c.startsWith(name + '='));
  return hit ? decodeURIComponent(hit.split('=')[1]) : '';
}

async function vpMeta(force) {
  if (VOICE_CACHE.meta && !force) return VOICE_CACHE.meta;
  const res = await fetch('/minecraft/voices.json', { credentials: 'same-origin' });
  VOICE_CACHE.meta = await res.json();
  return VOICE_CACHE.meta;
}

async function vpToken(scope, params) {
  const q = new URLSearchParams(Object.assign({ scope: scope }, params || {}));
  const res = await fetch('/minecraft/voices/token.json?' + q.toString(),
...
// Cloned voices and current assignments, read with a token that can only read.
// Cached briefly so flipping between cards is not a burst of requests.
async function vpRemote(force) {
  if (!force && VOICE_CACHE.remote && Date.now() - VOICE_CACHE.at < 20000) {
    return VOICE_CACHE.remote;
  }
  const t = await vpToken('read');
  const opts = { headers: { 'X-Upload-Token': t.token } };
  const [voices, assignments] = await Promise.all([
    fetch(t.endpoint + '/voices', opts).then(r => r.ok ? r.json() : { clones: [] }),
    fetch(t.endpoint + '/assignments', opts).then(r => r.ok ? r.json() : {}),
  ]);
  VOICE_CACHE.remote = {
    endpoint: t.endpoint,
    clones: (voices && voices.clones) || [],
    assignments: assignments || {},
  };
  VOICE_CACHE.at = Date.now();
  return VOICE_CACHE.remote;
}

function vpSay(panel, msg, kind) {
  const el = panel.querySelector('.vp-status');
  if (!el) return;
  el.textContent = msg || '';
  el.className = 'vp-status' + (msg ? ' on ' + (kind || 'ok') : '');
}

async function initVoicePanel(panel, p) {
  const root = panel.querySelector('.voice-panel');
  const body = panel.querySelector('.vp-body');
  if (!root || !body) return;
  ...
  try {
    ...
    body.innerHTML = '<p class="rp-none">the voice service is unreachable</p>';
    return;
  }

  if (!meta.configured) {
    body.innerHTML = '<p class="rp-none">chat voices are not configured on this site yet</p>';
    return;
  }
  if (!meta.unlocked) return vpRenderLock(panel, p, meta);
  vpRenderPicker(panel, p, meta);
}

// The shared server password, held in the session once entered. Anyone with it
// can set anyone's voice, which is how the server is actually run.
function vpRenderLock(panel, p, meta) {
  const body = panel.querySelector('.vp-body');
  body.innerHTML = `
    <div class="vp-lock">
      <p>Enter the server password to pick a voice or clone your own.</p>
      <input type="password" class="vp-pw" placeholder="Password" autocomplete="current-password">
      <button class="vp-btn vp-unlock">Unlock</button>
    </div>`;

  const input = body.querySelector('.vp-pw');
  const go = async () => {
    vpSay(panel, 'Checking…', 'ok');
    try {
      const res = await fetch('/minecraft/voices/unlock', {
        method: 'POST',
        credentials: 'same-origin',
        headers: { 'Content-Type': 'application/json', 'X-CSRFToken': vpCookie('csrftoken') },
        body: JSON.stringify({ password: input.value }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || 'that did not work');
      vpSay(panel, '');
      vpRenderPicker(panel, p, await vpMeta(true));
    } catch (e) {
      vpSay(panel, e.message, 'err');
    }
  };
  body.querySelector('.vp-unlock').addEventListener('click', go);
  input.addEventListener('keydown', e => { if (e.key === 'Enter') go(); });
}

async function vpRenderPicker(panel, p, meta) {
  const body = panel.querySelector('.vp-body');
  body.innerHTML = `
    <div class="vp-section">
      <div class="vp-head"><span>PRESETS</span><b class="vp-current">&mdash;</b></div>
      <div class="vp-grid vp-presets"></div>
    </div>
    <div class="vp-section vp-clones-wrap" hidden>
      <div class="vp-head"><span>CLONED</span></div>
      <div class="vp-grid vp-clones"></div>
    </div>
    <div class="vp-section">
      <div class="vp-head"><span>CLONE YOUR VOICE</span></div>
      <p class="vp-hint">
        Record 5&ndash;20 seconds of yourself talking normally, somewhere quiet, one voice only.
        Type out exactly what you said &mdash; the transcript matching the audio matters more
        than the clip being long.
      </p>
      <input type="text" class="vp-cid" placeholder="voice id, e.g. nick" maxlength="32">
      <input type="text" class="vp-cname" placeholder="display name" maxlength="40">
      <input type="file" class="vp-cfile" accept="audio/*">
      <textarea class="vp-ctext" placeholder="Exact transcript of the recording"></textarea>
      <button class="vp-btn vp-cgo">Upload and clone</button>
    </div>`;

  const grid = body.querySelector('.vp-presets');
  const uuid = (p.uuid || '').toLowerCase();

  let remote = { clones: [], assignments: {} };
  try {
    remote = await vpRemote();
  } catch (e) {
    vpSay(panel, 'Could not reach the voice service: ' + e.message, 'err');
  }

  const assigned = (remote.assignments[uuid] || {}).voice || null;

  const chip = (id, name, desc, cloned) => {
    const el = document.createElement('button');
    el.className = 'vp-chip' + (id === assigned ? ' on' : '') + (cloned ? ' cloned' : '');
    el.dataset.voice = id;
    const b = document.createElement('b');
    b.textContent = name;                 // player-authored for clones
    const i = document.createElement('i');
    i.textContent = desc || '';
    el.appendChild(b);
    el.appendChild(i);
    el.addEventListener('click', () => vpAssign(panel, p, id, name));
    return el;
  };

  (meta.presets || []).forEach(pr => grid.appendChild(chip(pr.id, pr.name, pr.desc, false)));

  if (remote.clones.length) {
    body.querySelector('.vp-clones-wrap').hidden = false;
    const cg = body.querySelector('.vp-clones');
    remote.clones.forEach(c => cg.appendChild(chip(c.id, c.name || c.id, 'cloned voice', true)));
  }

  vpMarkCurrent(panel, p, assigned, meta, remote);
  body.querySelector('.vp-cgo').addEventListener('click', () => vpClone(panel, p));
}

function vpMarkCurrent(panel, p, voiceId, meta, remote) {
  const label = panel.querySelector('.vp-current');
  let name = 'not set';
  if (voiceId) {
    const preset = (meta.presets || []).find(x => x.id === voiceId);
    const cloned = (remote.clones || []).find(x => x.id === voiceId);
    name = preset ? preset.name : (cloned ? (cloned.name || cloned.id) : voiceId);
  }
  if (label) label.textContent = name;

  // Mirror it onto the card tab so you can see it without opening the panel.
  const idx = (SEASONS[discIdx].roster || []).indexOf(p);
  const tab = document.getElementById(`rc-voice-${idx}`);
  if (tab) tab.textContent = voiceId ? name : '—';
}

async function vpAssign(panel, p, voiceId, name) {
  vpSay(panel, 'Setting…', 'ok');
  try {
    const t = await vpToken('assign', { uuid: p.uuid });
    const res = await fetch(t.endpoint + '/assignments/' + encodeURIComponent(t.subject), {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'X-Upload-Token': t.token },
      body: JSON.stringify({ voice: voiceId }),
    });
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.detail || ('the service said ' + res.status));
    }

    panel.querySelectorAll('.vp-chip').forEach(c =>
      c.classList.toggle('on', c.dataset.voice === voiceId));
    const label = panel.querySelector('.vp-current');
    if (label) label.textContent = name;
    const idx = (SEASONS[discIdx].roster || []).indexOf(p);
    const tab = document.getElementById(`rc-voice-${idx}`);
    ...
    // Keep the cache honest so reopening the panel shows the new pick.
    if (VOICE_CACHE.remote) {
      VOICE_CACHE.remote.assignments[(p.uuid || '').toLowerCase()] = { voice: voiceId };
    }
    vpSay(panel, 'Set to ' + name + '. It takes effect in game within a minute.', 'ok');
  } catch (e) {
    vpSay(panel, e.message === 'locked' ? 'Session expired, reopen the panel.' : e.message, 'err');
  }
}

async function vpClone(panel, p) {
  const body = panel.querySelector('.vp-body');
  const id = (body.querySelector('.vp-cid').value || '').trim().toLowerCase();
  const name = (body.querySelector('.vp-cname').value || '').trim();
  const text = (body.querySelector('.vp-ctext').value || '').trim();
  const file = body.querySelector('.vp-cfile').files[0];
  const btn = body.querySelector('.vp-cgo');

  if (!/^[a-z0-9_-]{2,32}$/.test(id)) {
    return vpSay(panel, 'Voice id must be 2-32 characters: a-z, 0-9, - or _.', 'err');
  }
  if (!name) return vpSay(panel, 'Give it a display name.', 'err');
  if (!file) return vpSay(panel, 'Pick an audio file.', 'err');
  if (file.size > 20 * 1024 * 1024) return vpSay(panel, 'That file is over 20 MB.', 'err');
  if (!text) return vpSay(panel, 'The transcript is required, and it has to match.', 'err');

  btn.disabled = true;
  vpSay(panel, 'Uploading and building the voice…', 'ok');
  try {
    const t = await vpToken('clone', { id: id });
    const form = new FormData();
    form.append('voice_id', t.subject);
    form.append('name', name);
    form.append('ref_text', text);
    form.append('audio', file);

    const res = await fetch(t.endpoint + '/voices/clone', {
      method: 'POST',
      headers: { 'X-Upload-Token': t.token },
      body: form,
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(data.detail || ('upload failed (' + res.status + ')'));

    vpSay(panel, 'Cloned. Selecting it now…', 'ok');
    await vpRemote(true);
    await vpAssign(panel, p, data.id, data.name || data.id);
    vpRenderPicker(panel, p, await vpMeta());
  } catch (e) {
    vpSay(panel, e.message, 'err');
  } finally {
    btn.disabled = false;
...
// is what makes a boss killed three times still worth one star.
// What each badge is, in the pack's own words: these are the exact labels
// boss_rewards.js prints in chat when one goes down, so a player reads the
// same grade here that they already saw in the kill announcement. Tier 4 was
...
// of the jar and the pack's datapack, and this draws it rather than drawing
// a diagram of it. Seven classes radiating from the middle, 686 nodes, 784
// connections.
//
