...
carries a `capacity` and it is currently ten. Whatever the tenth message pushes
off the end is gone from the server's side for good, so reading that file is
only ever a look at the last few seconds of talk.
...
Two consequences worth stating plainly:
  ...
  * How often this runs decides how much is kept. Ten messages is a busy
    minute on a full server, so a poll every thirty seconds keeps everything
    and a poll every ten minutes keeps a tenth of it. Nothing here can recover
    what rolled off between two reads - it was never sent to us - and nothing
    here pretends otherwise: a gap is simply absent, not marked.
...
def merge(data_dir, payload):
    ...
    # the stamp moves whether or not anything was said, so the box can tell
    # "nobody is talking" from "we have not looked in an hour"
